...
- **v4** : documentation utilisateur du CLI (session.md, tâche 9) créée dans `doc/cli/README.md` (commandes) et `doc/cli/format-contenu.md` (format `content.md`, cascade de configuration, résolution des chemins, modèles disponibles, convention de marge de sécurité).
- **v5** : marge du bas de `title-qr` mesurée à ~0.22cm (session.md, tâche 10, même méthode `convert -trim` que la tâche 9), jugée trop petite. Correctif : `yshift` du bloc événement/date/lieu porté de 0.2cm à 0.5cm (valeur minimale par défaut), police réduite de `\footnotesize` à `\scriptsize` pour compacter le bloc. Remesuré après correctif : marge basse 0.5cm.
- **v6** (courante) : réalignement complet de `title-qr` (session.md, tâche 11). Blocs réordonnés par importance décroissante (titre > photo+QR > nom+titre auteur > infos événement), espacement égal (`\NouBlocGap`) au-dessus/en-dessous du bloc photo+QR, bloc infos événement compacté (2 lignes resserrées), emoji 🐧 réintroduit près de « Martial Bigras ».
  - **Emoji** : `render_slide.lua` enveloppe désormais automatiquement le pingouin (U+1F427) dans `\NouEmojiFont` (police système `Noto Color Emoji`, rendu Harfbuzz, déclarée dans `preamble.tex` avec repli silencieux si absente) — les autres polices de la présentation n'ont pas ce glyphe, d'où sa disparition silencieuse constatée en tâche 5/6.
  - **Bug découvert et corrigé en cours de route** : le bloc infos événement était resté un `tikz overlay` ancré en absolu (hérité de `title.tex`), indépendant du flux normal des autres blocs désormais en `\vspace` fixe. Une fois les blocs réordonnés, le bloc auteur (flux) chevauchait visuellement le bloc infos événement (overlay absolu) — deux systèmes de positionnement indépendants qui ne se voient pas l'un l'autre. Corrigé en supprimant le `tikz overlay` : tous les blocs sont maintenant en flux normal séquentiel, ce qui élimine structurellement ce risque de chevauchement.
  - **Compromis mesuré** : avec 4 blocs empilés (au lieu de 3) et l'espacement égal imposé autour du bloc photo+QR, le budget vertical disponible (9cm de page) est plus serré. Marge du haut ramenée à la convention 0.6cm déjà utilisée par les autres gabarits du deck (`plain`/`tableau`/`equation`/`remerciements`) plutôt que 1.4cm, pour libérer de l'espace. Marges gauche/droite toujours à 1.4cm.
- **v7** (courante) : correction d'une confusion introduite en v6 (session.md, tâche 12). L'ordre d'importance communicationnelle (bloc1 > bloc2 > bloc3 > bloc4, tâche 11) avait été appliqué à tort comme ordre d'apparition **vertical** des blocs. L'humain précise : ce sont deux dimensions distinctes — l'ordre d'apparition vertical devait rester inchangé (bloc1 titre, bloc3 auteur, bloc2 photo+QR, bloc4 infos événement), l'ordre d'importance ne sert qu'à doser l'emphase/l'espacement, pas la position. Corrigé : blocs auteur et photo+QR permutés dans `title-qr.tex` pour revenir à l'ordre d'apparition d'origine, `\NouBlocGap` (espacement égal) toujours appliqué autour du bloc photo+QR mais désormais entre bloc3→bloc2 et bloc2→bloc4. Remesuré : marge haute 0.93cm, marge basse 0.34cm, aucun `Overfull \vbox`. Vérifié visuellement.
